...
func (r *SlidingWindowRepository) GetRateLimit(ctx context.Context, identifier string) (*models.RateLimit, error) {
...
func (r *SlidingWindowRepository) CreateRateLimit(ctx context.Context, identifier string, windowSize int) (*models.RateLimit, error) {
	now := time.Now()
...
func (r *SlidingWindowRepository) IncrementRateLimit(ctx context.Context, rateLimit *models.RateLimit) error {
	rateLimit.Count++
	rateLimit.LastRequest = time.Now()
	return r.db.Save(rateLimit).Error
}

func (r *SlidingWindowRepository) ResetRateLimit(ctx context.Context, rateLimit *models.RateLimit, windowSize int) error {
	now := time.Now()
	rateLimit.Count = 1
